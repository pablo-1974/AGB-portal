"""Rutas de la app Buzón Listados."""
...
from buzones.deps import (
    ...
    require_listados_buzon_marcar,
    require_listados_buzon_staff,
)
from buzones.listados import types as listados_types
from context import ctx
from db import listados_feedback as listados_db
from utils.enums import PERM_BUZONES_MARCAR_LEIDO_LISTADOS
from utils.permissions import has_permission

_log = logging.getLogger(__name__)

router = APIRouter(prefix="/buzones/listados", tags=["buzones-listados"])

URL_BASE = "/buzones/listados"
HEADER_TITLE = "Listados"
APP_TITLE = "Listados"
APP_INTRO = (
    "Comunicación de posibles errores y sugerencias de mejora "
    "en las consultas de listados del portal."
)
_MIN_MENSAJE = 10
...
def _app_ctx(request: Request, user: dict, **extra):
    return ctx(
        ...
        can_mark_read=has_permission(user, PERM_BUZONES_MARCAR_LEIDO_LISTADOS),
        **extra,
    )
...
def _tipos_enviar() -> list[dict]:
    return [
        ...
        for tid, label in listados_types.TIPO_LABELS.items()
    ]
...
def enviar_get(request: Request, user: dict = Depends(require_buzones_user)):
    ...
    if not listados_types.is_valid_tipo(tipo):
        return _templates(request).TemplateResponse(
            "listados_buzon/enviar_elegir.html",
            _app_ctx(
                request,
                user,
    ...
    return _templates(request).TemplateResponse(
        "listados_buzon/enviar.html",
        _app_ctx(
            request,
            user,
            title=f"{listados_types.tipo_label(tipo)} · {APP_TITLE}",
            tipo=tipo,
            tipo_label=listados_types.tipo_label(tipo),
            status=status,
        ),
    )
...
def enviar_post(
    ...
    try:
        listados_db.insert_feedback(user_id=int(user_id), tipo=tipo, mensaje=text)
    except Exception:
        _log.exception("Error guardando aviso buzón listados user=%s", user_id)
        return RedirectResponse(
            f"{URL_BASE}/enviar?tipo={tipo}&status=error", status_code=303
        )
    ...
    if user_id is not None:
        try:
            mensajes = rows_for_list(
                listados_db.list_feedback_for_user(user_id=int(user_id)),
                tipo_labels=listados_types.TIPO_LABELS,
            )
        except Exception:
            _log.exception("Error listando mis avisos buzón listados")

    return _templates(request).TemplateResponse(
        "listados_buzon/mis_mensajes.html",
        _app_ctx(
            request,
            user,
...
def listar_mensajes(request: Request, user: dict = Depends(require_listados_buzon_staff)):
    status = (request.query_params.get("status") or "").strip()
    mensajes: list[dict] = []
    try:
        mensajes = rows_for_list(
            listados_db.list_all_feedback(),
            tipo_labels=listados_types.TIPO_LABELS,
            include_user=True,
        )
    except Exception:
        _log.exception("Error listando avisos buzón listados")

    unread = [m for m in mensajes if not m["is_read"]]
    read = [m for m in mensajes if m["is_read"]]

    return _templates(request).TemplateResponse(
        "listados_buzon/listar_mensajes.html",
        _app_ctx(
            request,
            user,
