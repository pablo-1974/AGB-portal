"""Avisos del buzón Funcionamiento del portal (tabla propia)."""

from __future__ import annotations

from db.connection import get_db

TABLE = "funcionamiento_portal_feedback"
_schema_ready = False
...
def ensure_funcionamiento_portal_feedback_schema() -> None:
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                f"CREATE INDEX IF NOT EXISTS idx_fpfb_sent_at ON {TABLE} (sent_at DESC)"
            )
            cur.execute(
                f"CREATE INDEX IF NOT EXISTS idx_fpfb_user_id ON {TABLE} (user_id)"
            )
            cur.execute(
                f"CREATE INDEX IF NOT EXISTS idx_fpfb_read_at ON {TABLE} (read_at)"
            )
            cur.execute(
                f"""
                ALTER TABLE {TABLE}
