"""Avisos del buzón Listados (errores y sugerencias sobre consultas de listados)."""

from __future__ import annotations

from db.connection import get_db

TABLE = "listados_feedback"
_schema_ready = False


def ensure_listados_feedback_schema() -> None:
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                CREATE TABLE IF NOT EXISTS {TABLE} (
                    ...
                    tipo TEXT NOT NULL CHECK (tipo IN ('error_listados', 'sugerencia_listados')),
            ...
            cur.execute(
                f"CREATE INDEX IF NOT EXISTS idx_listfb_sent_at ON {TABLE} (sent_at DESC)"
            )
            cur.execute(
                f"CREATE INDEX IF NOT EXISTS idx_listfb_user_id ON {TABLE} (user_id)"
            )
            cur.execute(
                f"CREATE INDEX IF NOT EXISTS idx_listfb_read_at ON {TABLE} (read_at)"
            )
            cur.execute(
                f"""
                ALTER TABLE {TABLE}
...
def count_unread_feedback() -> int:
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute(
                f"SELECT COUNT(*) AS n FROM {TABLE} WHERE read_at IS NULL"
            )
            row = cur.fetchone()
