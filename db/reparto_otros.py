"""Otros conceptos de reparto por departamento (Reparto)."""
...
def ensure_reparto_otros_schema() -> None:
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                f"""
                CREATE INDEX IF NOT EXISTS idx_reparto_otros_depto
...
def list_otros(departamento_abrev: str) -> list[dict]:
...
def delete_otro(*, fila_id: int, departamento_abrev: str) -> bool:
    ensure_reparto_otros_schema()
    key = (departamento_abrev or "").strip()
    if not key or int(fila_id) <= 0:
        return False
